"""
order-ci-generator-us: 根据销售订单生成 Commercial Invoice (CI) 美元版文件
用法:
  python make_ci_us.py --order_ids "SO202602241708360223"
  python make_ci_us.py --order_ids "SO202602241708360223,SO2026022511215330b3"
  python make_ci_us.py --order_ids "SO202602"  # 模糊匹配
...
def load_config():
    ...
    if not config_path or not os.path.exists(config_path):
        return {
            ...
            "template_name": "CI_template_US.xlsx"
        }
...
def query_orders_by_ids(conn, order_ids):
    """根据订单ID精确查询"""
    ...
    for r in rows:
        d = dict(r)
        # 美元价格直接使用price_usd
        d["calculated_price_usd"] = float(d.get("price_usd") or 0)
        results.append(d)
    return results


def query_orders_fuzzy(conn, keyword):
    """根据订单编号模糊匹配"""
    sql = """
    SELECT
        o.order_id, o.order_no, o.order_date, o.cli_id,
...
def generate_ci(orders, template_path, output_path, cli_name):
    ...
    # B10 = 国家
    ws.cell(10, 2).value = first_order.get("region", "") or "USA"
    # F10 = 电话
    ...
    for idx, order in enumerate(orders):
        ...
        # F列 = Unit Price (USD)
        price_usd = order.get("calculated_price_usd", 0)
        try:
            price_usd = float(price_usd)
        except:
            price_usd = 0
        ws.cell(row, 6).value = price_usd
        if price_usd:
            ws.cell(row, 6).number_format = '#,##0.00'

        # G列 = Total (price_usd * qty)
        total = price_usd * qty
    ...
    ws.cell(actual_total_amount_row, 7).number_format = '#,##0.00'
...
def main():
    parser = argparse.ArgumentParser(description="生成 Commercial Invoice 美元版 (order-ci-generator-us)")
    ...
    try:
        orders = query_orders_by_ids(conn, order_ids)

        if not orders and len(order_ids) == 1:
            orders = query_orders_fuzzy(conn, order_ids[0])
            if orders:
